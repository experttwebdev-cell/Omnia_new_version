...
interface ChatMessage {
  ...
  content: string;
  products?: any[];
  summary?: any; // ✅ Synthèse de recherche ajoutée
  selectedProduct?: any;
  timestamp: string;
  ...
  const initializeConversation = async (storeId: string) => {
    try {
      ...
      const welcomeContent = chatSettings?.chat_welcome_message ||
        "Bonjour 👋 Je suis OmnIA, votre assistant shopping intelligent. Que cherchez-vous aujourd’hui ? (ex: table basse, canapé, montre...)";
  ...
  const saveMessage = async (message: ChatMessage) => {
    ...
    try {
      await supabase.from('chat_messages').insert({
        ...
        summary: message.summary || null, // ✅ Sauvegarde du résumé
        ...
        {messages.map((message, index) => {
          ...
          return (
            <div key={index}>
              ...
              {/* ✅ Résumé de recherche */}
              {message.summary && (
                <div className="ml-11 mt-3 flex flex-wrap items-center gap-2 bg-gradient-to-r from-blue-100 to-purple-100 border border-blue-200 rounded-lg px-4 py-2 text-sm text-gray-800 shadow-sm">
                  <span className="font-semibold text-blue-700">📊 Résumé :</span>
                  {message.summary.categories?.length > 0 && (
                    <span>🪑 {message.summary.categories.join(', ')}</span>
                  )}
                  {message.summary.styles?.length > 0 && (
                    <span>🎨 {message.summary.styles.join(', ')}</span>
                  )}
                  {message.summary.materials?.length > 0 && (
                    <span>🧱 {message.summary.materials.join(', ')}</span>
                  )}
                  {message.summary.colors?.length > 0 && (
                    <span>🌈 {message.summary.colors.join(', ')}</span>
                  )}
                  {message.summary.hasPromo && (
                    <span className="text-green-700 font-medium">💸 Promo dispo</span>
                  )}
                </div>
              )}
...
function ChatInput({ loading, inputMessage, setInputMessage, handleSendMessage, handleKeyPress, inputRef }: any) {
  return (
    ...
    <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden hover:shadow-xl transition group">
      <div className="relative w-full h-40 bg-gray-100 overflow-hidden cursor-pointer" onClick={onViewDetails}>
        ...
        {product.compare_at_price && Number(product.compare_at_price) > Number(product.price) && (
          <div className="absolute top-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded-md font-semibold shadow">
