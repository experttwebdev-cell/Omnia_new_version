...
export function SignUp() {
  ...
  const validateStep = (step: number): boolean => {
    const stepFields: { [key: number]: (keyof SignUpForm)[] } = {
      ...
      4: [] // Step 4 is payment, no form validation needed
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        // Create seller profile with subscription_plan_id
          ...
          .insert([
            {
              ...
              role: 'seller',
              status: 'trial',
              subscription_plan_id: form.selectedPlan,
                      ...
                      >
                        ...
                        <ul className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-gray-700">
                          ...
                          <li className="flex items-start gap-2">
                            <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                            <span>
                              {plan.blog_articles_limit === -1 ? 'Articles blog illimités' : `${plan.blog_articles_limit} articles blog/mois`}
                            </span>
                          </li>
                          <li className="flex items-start gap-2">
                            <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                            <span>
                              {plan.chat_messages_limit === -1 ? 'Messages chat illimités' : `${plan.chat_messages_limit} messages chat/mois`}
                            </span>
                          </li>
                          <li className="flex items-start gap-2">
                            <CheckCircle className="w-4 h-4 text-green-500 flex-shrink-0 mt-0.5" />
                            <span>
                              {plan.seo_optimizations_limit === -1 ? 'SEO illimité' : `${plan.seo_optimizations_limit} optimisations SEO/mois`}
                            </span>
                          </li>
            ...
            {step === 4 && (
              <div className="space-y-6 animate-fade-in">
                <div className="text-center mb-6">
                  ...
                  <p className="text-gray-600">
                    Dernière étape avant de commencer votre essai gratuit de 14 jours
                  </p>
                </div>

                {/* Plan Summary */}
                {plans.find(p => p.id === form.selectedPlan) && (() => {
                  const selectedPlan = plans.find(p => p.id === form.selectedPlan)!;
                  const price = form.billingPeriod === 'annual' ? selectedPlan.price_annual : selectedPlan.price_monthly;
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            Facturation {form.billingPeriod === 'annual' ? 'annuelle' : 'mensuelle'}
                          </p>
                        </div>
                        <div className="text-right">
                          <div className="text-3xl font-bold text-gray-900">€{price}</div>
                          <div className="text-sm text-gray-600">
                            /{form.billingPeriod === 'annual' ? 'an' : 'mois'}
                          </div>
                        </div>
                      </div>
                      ...
                      {form.billingPeriod === 'annual' && (
                        <div className="bg-green-100 border border-green-300 rounded-lg p-3 mb-4">
                          <p className="text-sm text-green-800 font-medium flex items-center gap-2">
                            ...
                            Vous économisez €{((parseFloat(selectedPlan.price_monthly) * 12) - parseFloat(selectedPlan.price_annual)).toFixed(2)} par an
                          </p>
                        </div>
                      )}
                ...
                <div className="bg-white rounded-lg border border-gray-200 p-4">
                  <h5 className="font-semibold text-gray-900 mb-3">Récapitulatif du compte</h5>
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-yellow-800">
                        Vous serez redirigé vers Stripe pour finaliser le paiement. Votre carte ne sera pas débitée pendant les 14 jours d'essai.
                      </p>
                    </div>
                  </div>
                </div>
